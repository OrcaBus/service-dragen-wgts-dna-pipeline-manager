...
generating a draft for a somatic wgts dna workflow. Omit setting the TUMOR_LIBRARY_ID variabler for running a germline only workflow.
...
From the dragen WGTS DNA DRAFT event, we populate all inputs to make another draft event, complete with all required inputs.
...
Take in a dragen WGTS DNA DRAFT event, make sure it's valid, and then generate a READY event for the dragen WGTS DNA pipeline.
...
- **`./app`**: Contains the main application logic. This includes the lambda scripts, the event schemas and the step functions in template ASL format.
...
- **`./infrastructure`**: Contains the infrastructure code for the project:
  ...
  - **`./infrastructure/stage`**: Defines the stage stacks for different environments:
    ...
    - **`./infrastructure/stage/statefulApplicationStack.ts`**: The CDK stack entry point for provisioning stateful resources required by the application in `./app`.
    - **`./infrastructure/stage/statelessApplicationStack.ts`**: The CDK stack entry point for provisioning stateless resources required by the application in `./app`.
    - **`./infrastructure/stage/<service>`**: Interfaces and functions built per service, this might be lambda function builder constructs, or dynamodb table builder constructs specific to the application
